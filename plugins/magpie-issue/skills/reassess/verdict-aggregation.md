...
- `classification` — one of ten labels per
  [`issue-reproducer/verification.md`](../reproducer/verification.md).
- `nature` — one of five labels per
  [`issue-reproducer/verdict-composition.md` → *"The nature field"*](../reproducer/verdict-composition.md#the-nature-field).
...
- The adapted `reproducer.<ext>` (per
  [`issue-reproducer/extraction.md`](../reproducer/extraction.md)).
- The `verdict.json` capturing the observed behaviour.
- The original report's relevant excerpts in `description.md`.

[`issue-fix-workflow`](../fix-workflow/SKILL.md) accepts these
...
[`issue-reassess-stats`](../reassess-stats/SKILL.md) reads to
...
- [`issue-reassess-stats`](../reassess-stats/SKILL.md) —
  dashboard consumer.
- [`issue-fix-workflow`](../fix-workflow/SKILL.md) — next-skill
