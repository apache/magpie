...
| `announced` label set and the *"Public advisory URL"* body field populated, but no `<!-- apache-magpie: security-pages-checklist v1 -->` marker recorded on the tracker (check the body and every comment for the ticked `- [x]` form of the checklist item; the hand-off comment that carries the box is posted at the `pr merged` → `fix released` transition) | The [ASF security-committers policy](https://www.apache.org/security/committers.html) requires the project's public security pages to be updated after the announcement — that edit lives on the project website, so only a human can perform it and sync can only ask. Propose a short status comment reminding the release manager to update the project's security pages (URL from the `security_pages_url` key in [`<project-config>/project.md`](../../../../<project-config>/project.md#repositories); when the key is unset, use the plain phrase "the project's security pages" instead of a link) with `CVE_ID`, referencing the checklist item the hand-off comment carries and noting that ticking it marks the step done. The proposed comment itself embeds the `<!-- apache-magpie: security-pages-checklist v1 -->` marker inline in its own `- [ ] Project security pages updated with CVE_ID` box (the same form the hand-off comment carries; same marker-carrying idempotency pattern as the [RM hand-off comment](../../../../tools/cve-tool-vulnogram/release-manager-handoff-comment.md)) so that once the RM ticks the box — or once the comment is edited to `- [x]` after the RM confirms the update in a reply — the marker is recorded and the next sync run reads the signal as satisfied and re-proposes nothing. Sync never proposes editing the website itself; this is a reminder-and-record mechanism, not a write path. |
