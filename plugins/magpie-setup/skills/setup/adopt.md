...
1. **List them, and say what each would become.** Promoting is
   ...
   Always flag `adversarial-review.md` as personal: its `reviewers`
   reflect the model CLIs installed on this one machine. Promoting it
   with `mode: on-pr-create` would make every contributor's PR-opening
   skills send changes to those models' providers, paid for by each
   contributor, for anyone who has the CLIs — say so if the user ticks it.
   A contributor's own file still overrides the project's.
