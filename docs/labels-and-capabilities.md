...
| [`tools/bitbucket`](../tools/bitbucket/) | `contract:tracker` + `contract:change-request` | Bitbucket Cloud and Bitbucket Data Center forge bridge for repository metadata and pull-request discovery/fetching. Initial read-only adapter for Bitbucket pull requests and future Bitbucket Issues / Jira handoff coverage. |
