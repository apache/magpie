...
| | |
|---|---|
| **Status** | Built, except the two items under [Designed, not built](#designed-not-built) |
...
**Re-cutting `magpie-setup.svg`.** The committed recording opens with the
marketplace install, which is now a prerequisite with its own page. It should
begin at `/magpie-setup`, where the reader is. This needs a terminal, a scratch
project and a human; nothing can do it in CI. `record-svg.sh` prints the
corrected brief.
