...
- [Install, adopt, upgrade](#install-adopt-upgrade)
  - [What was wrong](#what-was-wrong)
  - [Decisions](#decisions)
  - [The adoption floor](#the-adoption-floor)
    - [The record](#the-record)
    - [`adopt` writes it](#adopt-writes-it)
    - [The derived wiring, and its merge rules](#the-derived-wiring-and-its-merge-rules)
    ...
    - [The surfaces that report on it](#the-surfaces-that-report-on-it)
  - [Installing: one page, one command per agent](#installing-one-page-one-command-per-agent)
  - [What a family page shows](#what-a-family-page-shows)
...
| | |
|---|---|
| **Status** | Built, except the one item under [Designed, not built](#designed-not-built) |
...
Two steps in [`tools/dev/preflight-block.md`](../../tools/dev/preflight-block.md),
...
[`docs/setup/marketplace-install.md`](../setup/marketplace-install.md) —
...
A twelfth check in
[`check-doc-sync.py`](../../tools/dev/check-doc-sync.py) keeps it that way: the
...
[`tools/dev/render-screenshot.sh`](../../tools/dev/render-screenshot.sh)
...
[`assets/quickstart/README.md`](../../assets/quickstart/README.md) says so
...
**Re-cutting `magpie-setup.svg` — resolved by deleting the problem.** The
committed recording opened with the marketplace install, which had become a
prerequisite with its own page, and fixing it needed a terminal, a scratch
project and a human. It is now generated by `render-wizard.py` instead: the
whole first run including the secure-agent setup, deterministic, checked like
everything else. The recorder is retired. Nothing in the repository is
captured any more, which also ends the standing risk that the one artefact
needing a human stays wrong because a human is hard to schedule.

**The per-skill first-run wizard.** Designed in full and deliberately not
started. The shared pre-flight answers "has this repository adopted Magpie at
all"; it does not answer "does *this skill* have the configuration it needs", so
a skill whose config file is absent proceeds against defaults. The design:

- each of the 65 skills carrying the pre-flight declares a `requires_config:`
  frontmatter list — the one to three files without which it would act on a
  guess. Everything else a skill reads is optional and degrades, which is
  already how skills are written;
- the *optional* set is derived, never hand-written: tooling greps each skill
  for `<project-config>/*` references and emits a map, with a `--fix` checker
  and a pre-commit hook, mirroring the `check-skill-preflight` pattern. It also
  validates that every file named exists in `projects/_template/`, so a skill
  cannot require config the framework cannot scaffold;
- the pre-flight gains **one** cheap check: are this skill's `requires_config`
  files present? If not, hand off to `/magpie-setup configure <skill>`;
- the wizard lives in **one** place — a `configure` sub-action — not propagated
  into 65 skill bodies. It auto-detects what the repository reveals, batches the
  remaining gaps into one structured question, writes and stages into
  `.apache-magpie-overrides/`, and returns control so the invoking skill
  continues. It is an interruption, not a destination. Idempotence needs no
  state file: the config file existing is the marker.
