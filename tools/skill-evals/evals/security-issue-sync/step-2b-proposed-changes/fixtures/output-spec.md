...
```json
{
  "items": [
    {
      "number": 1,
      "category": "labels" | "milestone" | "assignee" | "body_field" | "status_comment" | "reporter_unresponsive",
      "action": "<string>",
      "reason": "<string>"
    }
  ],
...
- `security_pages_reminder_proposed`: `true` when `items` contains the security-pages reminder status comment. Optional field — omit (or set `false`) when the advisory has not shipped yet (no `announced` label or empty *Public advisory URL*), or when the hand-off comment's security-pages checklist item is already ticked / the marker is recorded in ticked form anywhere on the tracker. The reminder fires at most once per tracker: a comment carrying the marker (ticked or not) suppresses any further reminder.
