...
# Shape emitted by tools/dev/skill-surface-hash.py: `sha256:` + a 16-char
# lowercase hex digest (hashlib.sha256(...).hexdigest()[:16]).
SURFACE_HASH_RE = re.compile(r"^sha256:[0-9a-f]{16}$")

# Shape emitted by tools/skill-token-count: a positive decimal token count.
MEASURED_TOKENS_RE = re.compile(r"^[1-9][0-9]*$")
...
def validate_frontmatter(path: Path, text: str, root: Path | None = None) -> Iterable[Violation]:
    ...
    # surface_hash is generated by tools/dev/skill-surface-hash.py — it must
    # never be hand-typed, so both failure messages below point at the
    # generator rather than describing a value to author.
    if "surface_hash" not in fm:
        yield Violation(
            path,
            1,
            "missing required frontmatter key: 'surface_hash' — this field is generated, "
            "run `python3 tools/dev/skill-surface-hash.py --fix` to stamp it",
        )
    elif not SURFACE_HASH_RE.fullmatch(fm["surface_hash"]):
        yield Violation(
            path,
            1,
            f"frontmatter surface_hash '{fm['surface_hash']}' is not 'sha256:' followed by 16 hex "
            "characters — this field is generated, run `python3 tools/dev/skill-surface-hash.py --fix` "
            "to regenerate it",
        )

    # measured_tokens is generated by tools/skill-token-count; whether it is
    # current is that tool's check, this one only rejects a malformed value.
    if "measured_tokens" in fm and not MEASURED_TOKENS_RE.fullmatch(fm["measured_tokens"]):
        yield Violation(
            path,
            1,
            f"frontmatter measured_tokens '{fm['measured_tokens']}' is not a positive integer — this "
            "field is generated, run `uv run --project tools/skill-token-count skill-token-count --write`",
        )
