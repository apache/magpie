...
def get_issue_attachments(config: BitbucketConfig, issue_id: str) -> dict[str, Any]:
    """Reject native Bitbucket issue attachment fetch for Data Center."""
    _ = (config, issue_id)
    msg = (
        "Bitbucket Data Center native issue attachments are not supported; use linked Jira coverage instead."
    )
    raise BitbucketError(msg)
...
def approve_pull_request(
    ...
    """Reject pull-request approval writes for Data Center for now."""
    _ = (config, pull_request_id)
    raise BitbucketError(
        "Bitbucket Data Center pull request approval writes are not supported by this command yet"
    )
...
def unapprove_pull_request(
    ...
    """Reject pull-request unapproval writes for Data Center for now."""
    _ = (config, pull_request_id)
    raise BitbucketError(
        "Bitbucket Data Center pull request approval writes are not supported by this command yet"
    )
...
def decline_pull_request(
    ...
    """Reject pull-request decline writes for Data Center for now."""
    _ = (config, pull_request_id)
    raise BitbucketError(
        "Bitbucket Data Center pull request decline writes are not supported by this command yet"
    )
