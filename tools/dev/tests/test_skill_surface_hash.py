...
# A declared block, generated by `check-shared-blocks.py` for a name other
# than the one auto block (`preflight`). Its heading is shared framework
# prose the moment a later task extracts one of these into a skill; it must
# be just as invisible to the fingerprint as the pre-flight block always
# was. This is the case a `PREFLIGHT_RE`-only strip cannot see: it fails
# without `strip_generated_regions()` folding in `DECLARED_RE` too.
DECLARED_BLOCK = (
...
def test_declared_block_heading_is_excluded_from_the_hash(tmp_path: Path) -> None:
    with_block = _make_skill(tmp_path / "with-block", SKILL + "\n" + DECLARED_BLOCK)
    without_block = _make_skill(tmp_path / "without-block", SKILL)

    _, anchors = MOD.surface_inputs(with_block)
    assert not any("Shared Widget Heading" in a for a in anchors)
    assert MOD.surface_hash(with_block) == MOD.surface_hash(without_block)


def test_declared_block_heading_in_detail_file_is_excluded_from_the_hash(tmp_path: Path) -> None:
    with_block = _make_skill(tmp_path / "with-block", SKILL, **{"guide.md": DECLARED_BLOCK})
    without_block = _make_skill(tmp_path / "without-block", SKILL, **{"guide.md": ""})
