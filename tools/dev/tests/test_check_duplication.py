...
# --- generated pre-flight sidecars are not scanned ---------------------------------


def test_discover_targets_skips_generated_sidecars_and_scans_their_source(
    tmp_path: Path,
) -> None:
    """65 byte-identical copies of one file would be 65x64 perfect-score
    pairs saying nothing except that propagation worked. The source is
    scanned in their place, exactly as the pre-flight block's is."""
    module = _load()
    skills = tmp_path / "skills"
    ...
    blocks.mkdir()
    preflight = tmp_path / "preflight-block.md"
    preflight.write_text("# Block\n")
    detail_source = tmp_path / "preflight-detail.md"
    detail_source.write_text("# Detail source\n")

    targets = module.discover_targets(skills, blocks, preflight, detail_source)
    names = [p.name for p in targets]

    assert module.PREFLIGHT_DETAIL_NAME not in [p.name for p in targets if p.parent.name == "alpha"]
    assert "SKILL.md" in names and "locks.md" in names
    assert detail_source in targets
    assert preflight in targets


def test_the_live_wired_scope_scans_no_sidecar() -> None:
    module = _load()
    targets = module.discover_targets()
    assert all(p.name != module.PREFLIGHT_DETAIL_NAME for p in targets)
